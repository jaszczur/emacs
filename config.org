...
- [[#important-to-load-first][Important to load first]]
  ...
  - [[#native-compilation-options][Native compilation options]]
...
#+begin_src emacs-lisp
    ...
    (defun j/compile-packages () 
      (interactive)
      (native-compile-prune-cache)
      (native-compile-async "~/.config/emacs/elpaca/builds/" 'recursively)
      (message "Compiling all Elpaca packages in background"))
#+end_src
...
#+begin_src emacs-lisp
    (use-package general
      ...
      (j/leader-keys
        ...
        "h e" '(:ignore t :wk "Emacs")
        "h e c" '(j/compile-packages :wk "Compile packages natively")
        "h e u" '(elpaca-update-all :wk "Update packages"))
...
#+begin_src emacs-lisp
    (use-package emacs
      ...
      (visible-bell t)            ;; Show sign instead of beep
      ...
      (save-place-mode 1) ;; Restore file location
...
** Sideline
Display errors inline (like virt_text in nvim).
...
Let's configure flycheck backend for sideline

#+begin_src emacs-lisp
    (use-package sideline-flycheck 
      ;; :ensure (:host github :repo "emacs-sideline/sideline-eglot")
      :hook 
      (flycheck-mode . sideline-mode)
      (flycheck-mode . sideline-flycheck-setup))
#+end_src

Let's configure also eglot backend for sideline
...
** Eldoc box - WiP
Display documentation in child frame instead of in the bottom. Also display when asked for (not always)
...
#+begin_src emacs-lisp
   ...
   ;       :general
   ;       (:keymaps 'eglot-mode-map
   ;                 "C-e" 'rex/eldoc-box-scroll-up
   ;                 "C-n" 'rex/eldoc-box-scroll-down
   ;                 "M-h" 'eldoc-box-eglot-help-at-point)
   ;       (:keymaps 'eglot-mode-map :states 'normal
   ;                 "S-k" 'eldoc-box-eglot-help-at-point)
   ;)
...
#+begin_src emacs-lisp
    (use-package eglot
      ...
      (eglot-report-progress t) ;; Disable lsp server logs (Don't show lsp messages at the bottom, java)
      ;; Manual lsp servers
      :config
...
#+begin_src emacs-lisp
    (use-package corfu
      ...
      (:keymaps 'corfu-popupinfo-map
                ...
                "C-S-e" 'corfu-popupinfo-scroll-down
...
#+begin_src emacs-lisp
    (use-package envrc
      ...
      :config
      (j/leader-keys :states '(normal) :keymaps '(envrc-mode-map)
        "d e" '(envrc-command-map :wk "Direnv")))
#+end_src
** Evil extensions
*** Commenting
